...
export class StepsFactory {
    ...
    generateStepProps(data) {
        ...
        return Object.assign({ stepProgress: data.progress.step - 1, image: data.image.display_file, motion1: data.motion.motion_1, motion2: data.motion.motion_2, sound: data.audio.mp3, duration: duration || data.audio.time, seekStart: data.audio.seekbar_start, isResultStep: data.question.is_result_step, talking: duration ? "boy" : "teacher" }, this.buildQuizData(data));
    }
    generateDuration(ev) {
        switch (ev) {
            case "1_second":
