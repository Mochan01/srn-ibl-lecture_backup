...
const quiz_correct = new URL("../../../assets/prod/quiz_correct.mp3", import.meta.url).toString();
...
const AnswerBtnWrapper = styled.div `
  ...
  `}
`;
export const QuizArea = ({ questions, correctIndex, $x, $y, $width, $height, onAnswer = () => { } }) => {
    // 解答ボタン 状態管理
    ...
    return (React.createElement(Main, { "$width": calcRatio(SIZE.QUIZ_AREA_W, $width), "$height": calcRatio(isMaxLen ? SIZE.QUIZ_AREA_FOUR_H : SIZE.QUIZ_AREA_TREE_H, $height), "$x": calcRatio(SIZE.W, $x), "$y": calcRatio(SIZE.H, $y), touchEnabled: !isAnswered, isMaxLen: isMaxLen },
        questions.map((x, i) => (React.createElement(QuizChoiceBtn, { key: i, mutation: chooseIndex === i
                ? QUIZ_CHOICE_BTN.ORANGE
                : QUIZ_CHOICE_BTN.WHITE, onClick: () => setChooseIndex(i), isCorrect: isAnswered ? i === correctIndex : null }, x))),
        React.createElement(AnswerBtnWrapper, { isMaxLen: isMaxLen },
            React.createElement(QuizAnswerBtn, { mutation: mutation, onClick: () => setAnswered(true) }))));
};
