...
export const QuizArea = ({ questions, correctIndex, $x = 0, $y = 0, $width = SIZE.QUIZ_AREA_W, $height = SIZE.QUIZ_AREA_H }) => {
    ...
    return (React.createElement(Main, { "$width": calcRatio(SIZE.QUIZ_AREA_W, $width), "$height": calcRatio(SIZE.QUIZ_AREA_H, $height), "$x": calcRatio(SIZE.W, $x), "$y": calcRatio(SIZE.H, $y) },
