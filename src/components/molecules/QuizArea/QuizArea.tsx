import React, { FC, useEffect, useMemo, useState, memo, Fragment, useCallback } from "react";
import styled from "styled-components";
import { QuizAnswerBtn, QUIZ_ANSWER_BTN } from "../../atoms/QuizAnswerBtn/QuizAnswerBtn";
import { QuizChoiceBtn, QuizChoiceBtnProps, QUIZ_CHOICE_BTN } from "../../atoms/QuizChoiceBtn/QuizChoiceBtn";
import { SIZE } from "../../../data/SIZE";
import { useSound } from "use-sound";
import { QuizProps } from "../../../variable_types/StepProps";
const quiz_correct
  = new URL("../../../assets/prod/quiz_correct.mp3", import.meta.url).toString();
import { useGetRefSize } from "../../../hooks/useGetRefSize";
...
const AnswerBtn = styled(QuizAnswerBtn)<IsMaxLen>(({ isMaxLen }) => `
...
export const QuizArea: FC<QuizAreaProps> = ({
  ...
  const mutation = typeof chooseIndex === "number"
    ? (isAnswer ? QUIZ_ANSWER_BTN.RED : QUIZ_ANSWER_BTN.WHITE)
...
const QuizBtnMemo: FC<QuizBtnMemoProps> = memo(({
  ...
  return <QuizChoiceBtn { ...{ mutation, sign, onClick } }>{ children }</QuizChoiceBtn>;
});
